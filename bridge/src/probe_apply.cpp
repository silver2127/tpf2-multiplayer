...
// id 3: StreetBuilder::UpdateEngine -- the UI street tool pushing its intent out
// to the engine, located via the assert string "!proposalData.proposal.proposal"
// (see docs/re/UI_CAPTURE_PATH.md). This is the candidate interception point for
// lockstep, because unlike applyProposal it belongs to the UI tool rather than
// being the shared downstream of the engine's own construction.
//
// The whole question this target exists to answer: does it fire ONLY when the
// player builds? applyProposal logged 89 commits in 139 s because towns build
// houses continuously. If UpdateEngine stays silent while towns grow and fires
// on a click, it is the right place to intercept; if towns trip it too, it is no
// better than what we already have.
//
// Steal 21: the prologue's instruction boundaries are 3,4,5,7,14,21,30 and 21 is
// both a real boundary and a size hook.cpp supports, with no RIP-relative bytes
// in that range. The function opens `mov rax, rsp`, which the trampoline
// re-executes -- safe only because the relay restores rsp to its entry value
...
static const Target TARGETS[] = {
    ...
    { 0x459ce0, 21, "StreetBuilder::UpdateEngine" },
};
static const int NUM_TARGETS = 4;
static const int BLOB_SIZE = 48;

extern "C" void ApplyRelayProbe();   // applyrelay_probe.asm
...
static volatile long g_hits[NUM_TARGETS] = { 0, 0, 0, 0 };
...
extern "C" void ApplyHandlerProbe(uint64_t rcx, uint64_t rdx, uint64_t r8, uint64_t r9, uint64_t id, uint64_t retAddr)
{
    ...
    __try {
        if (id == 0) {
            ...
            g_commits++;
            // WHO called us -- read straight off the stack by the relay.
            //
            // Static analysis found 24 call sites for this function. A lockstep
            // hook must cancel only the one a PLAYER's click travels through;
            // suppressing an internal engine caller would corrupt whatever that
            // path was doing. Static xrefs list all 24 without saying which
            // carries a click. This does, because we only reach here when the
            // player actually builds.
            //
            // RtlCaptureStackBackTrace was tried first and returned ZERO frames:
            // x64 unwinding is table-driven and this relay is VirtualAlloc'd
            // with no RUNTIME_FUNCTION data, so the walk cannot leave frame 0.
            {
                uintptr_t mbase = (uintptr_t)GetModuleHandleW(nullptr);
                if (retAddr > mbase)
                    Log("[probe]   caller rva=%llx\n",
                        (unsigned long long)(retAddr - mbase));
                else
                    Log("[probe]   caller raw=%llx (outside main module)\n",
            ...
            // StreetBuilder::UpdateEngine. Observation only -- the original
            // still runs, so builds behave normally.
            //
            // Timestamped so the pattern can be read against the applyProposal
            // commits: those arrive continuously because towns build houses. If
            // these arrive ONLY in a cluster around a click, this is the
            // player-specific point that applyProposal could never be.
            //
            // rcx is the builder `this` (its field accesses run out to 0xac8).
            Log("[probe] UPDATE_ENGINE #%ld t=%llums this=%llx caller_rva=%llx\n",
                g_hits[3], (unsigned long long)GetTickCount64(),
                (unsigned long long)rcx,
                (unsigned long long)(retAddr - (uintptr_t)GetModuleHandleW(nullptr)));
        }
...
{
    // REFUSE to install if a copy of this probe is already in the process.
    //
    // Injecting a second copy has silently wasted three measurement cycles. It
    // does not fail loudly: the newer DLL patches an address the older one has
    // already detoured, both open the same log path with "w" so their file
    // handles clobber each other, and the log ends up showing the OLD probe's
    // output -- which reads exactly like the new hook simply never fired. The
    // giveaway was "3/3 hooks installed" from a build that has four targets.
    //
    // A DLL stays loaded for the life of the process, so the only correct way to
    // change this probe is to restart the game. Say that instead of producing
    // uninterpretable data.
    HANDLE once = CreateMutexA(nullptr, TRUE, "tpf2_probe_apply_single_instance");
    if (once == nullptr || GetLastError() == ERROR_ALREADY_EXISTS) {
        ...
        snprintf(p2, sizeof(p2),
            ...
            "3710243057/recon/m4/out/tpf2_probe_REFUSED.log");
        FILE* f = _fsopen(p2, "a", _SH_DENYWR);
        if (f) {
            fputs("[probe] REFUSING to install: another copy of this probe is already "
                  "loaded in this process. Restart the game and inject once.\n", f);
            fclose(f);
        }
        return 0;
    }

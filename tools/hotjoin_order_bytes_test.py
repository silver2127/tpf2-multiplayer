...
assert [s[0] for s in sites] == ["candidates", "departures", "arrivals", "idle"], sites
...
VECTOR = {"candidates": "[rbp-71h]", "departures": "[rbx+78h+28h]", "arrivals": "[rbx+78h+68h]", "idle": "[r13+18h]"}
...
for idx, (name, rva_s, steal_s, expect_s, elen_s, relay_name, resume) in enumerate(sites):
    ...
    want = {"candidates": ["mov dword ptr [rbp - 0x79], 1"], "departures": ["lea rdx, [rsp + 0x28]"],
            ...
            "idle": ["mov rdx, qword ptr [r13 + 0x20]", "sub rdx, qword ptr [r13 + 0x18]"]}[name]

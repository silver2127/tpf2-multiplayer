...
entry for that id is dropped and ours takes its place. A subscription to an item
Steam has since removed still lists the id with an empty install folder; the
catalogue named it, the lobby saw it present, and the loader asserted
`!modDir.empty()` running its mod.lua at world load (two players, two removed
items, every join). The lobby publishes a row for every Workshop mod of the
shared save it finds on disk, catalogued or not, on both ends.

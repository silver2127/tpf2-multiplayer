#!/usr/bin/env python3
"""Verify the cumulative Linux Lua integration: Windows 0.7.0.3 + dev d8a3ce57."""
import argparse
import hashlib
...
REFERENCE = "d8a3ce570e429396ca45e32a753ab32a5e2bd88d"
INCOMING = REFERENCE
INCOMING_FILES = set()
...
def main():
    ...
    try:
        ...
        print(f"PASS: {len(expected)} Lua files: exact Windows 0.7.0.3 + dev d8a3ce57 except {len(MERGED_SHA256)} pinned cumulative merges")
        ...
        if bad_glyphs:
            return 1
        print(f"PASS: {len(expected_glyphs)} HUD glyph textures exact Windows 0.7.0.3 + dev d8a3ce57")
        return 0

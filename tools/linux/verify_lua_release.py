#!/usr/bin/env python3
"""Verify the cumulative Linux Lua integration: dev 23419163 (release 0.6)."""
import argparse
import hashlib
...
REFERENCE = "23419163881f81188559362401f62da091da4956"
INCOMING = REFERENCE
INCOMING_FILES = set()
...
def main():
    ...
    try:
        ...
        print(f"PASS: {len(expected)} Lua files: exact dev 23419163 (release 0.6) except {len(MERGED_SHA256)} pinned cumulative merges")
        ...
        if bad_glyphs:
            return 1
        print(f"PASS: {len(expected_glyphs)} HUD glyph textures exact dev 23419163 (release 0.6)")
        return 0

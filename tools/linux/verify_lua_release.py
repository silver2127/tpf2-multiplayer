#!/usr/bin/env python3
"""Verify the cumulative Linux Lua integration: Windows 0.7 + dev 0a35d0a8."""
import argparse
import hashlib
...
REFERENCE = "0a35d0a82e5d26ae9bccef3bf864d2b3a2de2ebd"
INCOMING = REFERENCE
INCOMING_FILES = set()
...
def main():
    ...
    try:
        ...
        print(f"PASS: {len(expected)} Lua files: exact Windows 0.7 + dev 0a35d0a8 except {len(MERGED_SHA256)} pinned cumulative merges")
        ...
        if bad_glyphs:
            return 1
        print(f"PASS: {len(expected_glyphs)} HUD glyph textures exact Windows 0.7 + dev 0a35d0a8")
        return 0

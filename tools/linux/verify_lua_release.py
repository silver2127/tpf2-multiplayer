#!/usr/bin/env python3
"""Verify the cumulative Linux Lua integration: Windows v0.6.1.26."""
import argparse
import hashlib
...
REFERENCE = "1eb3000202a191003033eb1293fb769402ead04f"
INCOMING = REFERENCE
INCOMING_FILES = set()
...
def main():
    ...
    try:
        ...
        print(f"PASS: {len(expected)} Lua files: exact Windows v0.6.1.26 except {len(MERGED_SHA256)} pinned cumulative merges")
        ...
        if bad_glyphs:
            return 1
        print(f"PASS: {len(expected_glyphs)} HUD glyph textures exact Windows v0.6.1.26")
        return 0

#!/usr/bin/env python3
"""Verify the cumulative Linux Lua integration: Windows v0.6.1.27."""
import argparse
import hashlib
...
REFERENCE = "66a584cb4a43eb8d7619fd55224ae229fdd04cba"
INCOMING = REFERENCE
INCOMING_FILES = set()
...
def main():
    ...
    try:
        ...
        print(f"PASS: {len(expected)} Lua files: exact Windows v0.6.1.27 except {len(MERGED_SHA256)} pinned cumulative merges")
        ...
        if bad_glyphs:
            return 1
        print(f"PASS: {len(expected_glyphs)} HUD glyph textures exact Windows v0.6.1.27")
        return 0

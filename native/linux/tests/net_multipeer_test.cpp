...
int main(int argc, char** argv) {
    ...
    socklen_t size=sizeof(address); assert(getsockname(relay,(sockaddr*)&address,&size)==0);
    ...
    for(int round=0;round<3;round++) {
        ...
        TestSleep(100); assert(count()==before);
        for(int i=0;i<peers;i++) {
        ...
        TestSleep(100); assert(pending()==1 && count()==before+peers);
        ...
        const auto deadline=NowMs()+1800;
        while(NowMs()<deadline) {
            fd_set fds; FD_ZERO(&fds); FD_SET(relay,&fds); timeval tv{0,20000};
            if(select(relay + 1,&fds,nullptr,nullptr,&tv)<=0) continue;
            Packet p{};
        ...
        TestSleep(50); assert(count()==before+peers);
    }

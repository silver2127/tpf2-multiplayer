// Adapted from upstream 0.5.5 transport tests; uses real POSIX UDP sockets.
...
int main() {
    int relay=socket(AF_INET,SOCK_DGRAM,IPPROTO_UDP);
    ...
    socklen_t size=sizeof(endpoint); assert(getsockname(relay,(sockaddr*)&endpoint,&size)==0);
    ...
    auto keepAliveUntil=[&](uint32_t session,uint32_t ack,std::function<bool()> predicate) {
        auto deadline=NowMs()+4000;
        while(!predicate() && NowMs()<deadline) { send(session,world,0,nullptr,ack,ack==NO_ACK?0:0xffffffff); TestSleep(50); }
        assert(predicate());
    };
    ...
    TestSleep(150); assert(pending()==1);          // OLD is still awaited: not yet silent for long enough
    ...
    TestSleep(150); assert(count()==2 && member(OLD));   // data before its floor is known: admitted, not delivered
    ...
    { uint64_t noPeer=0; Net_Stats(&noPeer,nullptr,nullptr,nullptr,nullptr);
      Net_QueueLine("to-nobody",world.c_str()); TestSleep(50);

...
int main() {
    ...
    for (const char* value : {"0", "7", "121", "-1", "15junk"}) {
        std::ofstream f(flags); f << "dedicated_load_delay_sec=" << value << '\n'; f.close();
        assert(dedicated::Read(flags).loadDelaySeconds == 45);
    }
    dedicated::Configure(flags, dir);

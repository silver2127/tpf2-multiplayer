...
auto& g_canonReady = tpf2mp_order_detail::active;
...
bool RelinkMap(uintptr_t map) noexcept
{
    ...
    if (!count) { uintptr_t head; std::memcpy(&head,reinterpret_cast<const void*>(map+0x10),sizeof(head)); return !head; }
...
void CanonSort(unsigned site, uintptr_t rbp) noexcept
{
    ...
    if (begin != end && !std::is_sorted(b, e)) {
        std::sort(b, e);
        c.reordered.fetch_add(1, std::memory_order_relaxed);
    }
    ...
    if (n == 1 || !(n & 0xffff)) {
        if (auto log = g_canonLog.load(std::memory_order_relaxed))
            log("[order-canon] alive: %s calls=%llu reordered=%llu refused=%llu last=%lld\n", kCanonSites[site].name,
                ...
                (unsigned long long)c.refused.load(std::memory_order_relaxed), (long long)((end - begin) / 4));
    }
}
}
...
static bool InstallOrderCanon(uintptr_t base, const char* buildId, bool (*install)(uintptr_t,void*,int,void**))
{

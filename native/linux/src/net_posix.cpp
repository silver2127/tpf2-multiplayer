...
// POSIX adaptation of native/src/net.cpp at upstream 0.5.3 (66da00d).
// The wire protocol and stream/epoch algorithms match Windows. Heap-backed
// state survives process-exit destructors while detached bridge threads stop.
...
static std::mutex& g_peerMtx = *new std::mutex;
...
static std::thread& g_thread = *new std::thread;
static std::atomic<bool> g_running{false};

static uint32_t g_nextSeq = 0;
static uint32_t g_session = 0;          // our id (set at init)
static std::map<uint32_t, Packet>& g_pending = *new std::map<uint32_t, Packet>;        // sent, awaiting ack
static std::map<uint32_t, uint64_t>& g_lastSent = *new std::map<uint32_t, uint64_t>;     // seq -> last send time
static std::queue<NetEvent>& g_outQueue = *new std::queue<NetEvent>;
static std::mutex& g_mtx = *new std::mutex;
static std::mutex& g_epochMtx = *new std::mutex; // outer lock: net iteration vs coordinated reset
static std::string& g_worldEpoch = *new std::string(32, '0');
static std::string& g_lobbyEpoch = *new std::string;
...
static std::map<uint32_t, PeerStream>& g_streams = *new std::map<uint32_t, PeerStream>; // protected by g_epochMtx
// A broadcast remains pending until EVERY recipient present at send time ACKs.
// Do not remove a missing recipient on a timeout: that would silently lose data.
static std::map<uint32_t, std::set<uint32_t>>& g_awaiting = *new std::map<uint32_t, std::set<uint32_t>>; // under g_mtx
static bool g_rosterFrozen = false;
...
static void ProcessAck(uint32_t sender, uint32_t ack, uint32_t bits)
{
    ...
    for (auto it = g_pending.begin(); it != g_pending.end();) {
        ...
        if (acked) g_awaiting[s].erase(sender);
        if (g_awaiting[s].empty()) {
            g_awaiting.erase(s);
            g_lastSent.erase(s);
...
{
    while (g_running) {
        // Net_SignalShutdown closes the socket and publishes -1 to
        ...
        timeval tv{0, 20000}; // POSIX select modifies its timeout.
        ...
        if (n > 0 && FD_ISSET(sock, &fds)) {
            ...
            if (got >= (int)sizeof(Header) && p.h.magic == MAGIC) {
                ...
                // just the most recently seen sender. A restarted process still
                // needs a fresh shared-save session before it can participate.
                if (found == g_streams.end()) {
                    if (g_rosterFrozen) continue;
              ...
              void (*deliverCb)(const char*))
{
    ...
    g_running = true;
    try { g_thread = std::thread(NetThread); }
    catch (...) { g_running = false; close(g_sock.exchange(-1)); g_localPort = 0; return false; }
    return true;
}
...
// Non-blocking half used during ELF teardown. Shutdown wakes the receiver;
// the explicit Net_Shutdown path joins it before releasing transport state.
void Net_SignalShutdown()
{
    g_running = false;
    const int socket = g_sock.exchange(-1);

...
// POSIX adaptation of native/src/net.cpp at upstream 0.5.5 (ecbaf1d).
...
static std::map<uint32_t, std::string>& g_worldNoted = *new std::map<uint32_t, std::string>;

static bool PeerAlive()
{
    return g_peerEverSeen && (NowMs() - g_lastRecvMs) < g_peerTimeoutMs;
}

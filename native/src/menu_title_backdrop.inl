...
static bool ReadTitleArt(const wchar_t* archive,std::vector<unsigned char>& pixels,int& w,int& h)
{
    FILE* f=nullptr; if(_wfopen_s(&f,archive,L"rb") || !f) return false;
    ...
    while(fread(head,1,30,f)==30 && TitleU32(head)==0x04034b50) {
        ...
        if((names && fread(&name[0],1,names,f)!=names) || _fseeki64(f,extra,SEEK_CUR)) break;
        if(name!="menu-bg_transport-fever_blurred.tga") {
            if(_fseeki64(f,size,SEEK_CUR)) break;
            continue;
        }
...
static void PrepareTitleBackdrop(int w,int h)
{
    ...
    if(!g_titleArtTried) {
        ...
        wchar_t* slash=wcsrchr(path,L'\\'); if(slash) slash[1]=0; else path[0]=0;
        wcscat_s(path,L"res\\textures\\ui\\ui.zip");
        if(!ReadTitleArt(path,g_titleArt,g_titleArtW,g_titleArtH))
            Log("[menu] native title artwork unavailable; using solid fallback\n");
    }

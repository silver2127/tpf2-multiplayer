...
// The proposal's added segments (a2+0x18..0x20), EVERY record: the old
// fixed-buffer form clamped silently to the caller's buffer, so a build past
// it shipped a partial edge list. Empty when the vector does not read.
static std::vector<Edge> DecodeEdges(uint64_t a2)
{
    std::vector<Edge> out;
    if (!Readable((void*)(a2 + 0x18), 16)) return out;
    uint64_t begin = 0, end = 0;
    memcpy(&begin, (void*)(a2 + 0x18), 8);
    memcpy(&end, (void*)(a2 + 0x20), 8);
    if (begin < 0x10000 || end <= begin) return out;
    uint64_t span = end - begin;
    if (span % 120 != 0 || span > PROPOSAL_SANITY_BYTES) { LogBadSpan("segment", a2 + 0x18, span, 120); return out; }
    if (!Readable((void*)begin, (size_t)span)) return out;
    const size_t n = (size_t)(span / 120);
    out.resize(n);
...
{
    std::vector<Node> out;
    if (!Readable((void*)a2, 16)) return out;
    uint64_t begin = 0, end = 0;
    memcpy(&begin, (void*)a2, 8);
    memcpy(&end, (void*)(a2 + 8), 8);
    if (begin < 0x10000 || end <= begin) return out;
    uint64_t span = end - begin;
    if (span % 24 != 0 || span > PROPOSAL_SANITY_BYTES) { LogBadSpan("node", a2, span, 24); return out; }
    if (!Readable((void*)begin, (size_t)span)) return out;
    const size_t n = (size_t)(span / 24);
    out.resize(n);
    const uint8_t* b = (const uint8_t*)begin;
    for (size_t i = 0; i < n; i++) {
...
// The road / track / upgrade branch of DeferHandler, in its own function for
// the same reason as ConstructionPlacementAtFactory: its node and edge lists
// are std::vectors of whatever size the proposal has.
static void RoadAtFactory(uint64_t rcx, uint64_t r8, bool isUpgrade)
{
    const std::vector<Node> nodes = DecodeNodes(r8);
    const std::vector<Edge> edges = DecodeEdges(r8);
    const int n = (int)nodes.size(), m = (int)edges.size();
    ...
    // addresses exactly the two removal vectors. Every record is decoded: the
    // old fixed buffers (256 nodes, 512 edges, 64 removed nodes) made the
    // decoders CAP silently, and an upgrade drag past the cap would have
    // shipped every add against a truncated removal list -- the peer adding
    // edges on top of the ones it never removed.
    const std::vector<Node> rmNodes = DecodeNodes(r8 + 0x30);
    const std::vector<Edge> rmEdges = DecodeEdges(r8 + 0x30);
    const int rn = (int)rmNodes.size(), re = (int)rmEdges.size();
                                 ...
                                 uint64_t id, uint64_t retAddr, uint64_t calleeRsp)
{
    ...
    if (caller != CALLER_BUILDPROPOSAL && !isUpgrade) {
        ...
        if (luaReplay) {
            ...
            __try {
                uint64_t b = 0;
                an = (int)(ReadVec(r8 + 0x00, &b, PROPOSAL_SANITY_BYTES) / 24);
                ae = (int)(ReadVec(r8 + 0x18, &b, PROPOSAL_SANITY_BYTES) / 120);
                rn = (int)(ReadVec(r8 + 0x30, &b, PROPOSAL_SANITY_BYTES) / 24);
                re = (int)(ReadVec(r8 + 0x48, &b, PROPOSAL_SANITY_BYTES) / 120);
            } __except (EXCEPTION_EXECUTE_HANDLER) {
                an = ae = rn = re = -1;
            }
            ...
            if (an >= 1 || rn >= 1) {
                __try {
                    uint64_t ab = 0, rb = 0;
                    ReadVec(r8 + 0x00, &ab, PROPOSAL_SANITY_BYTES);
                    ReadVec(r8 + 0x30, &rb, PROPOSAL_SANITY_BYTES);
                    for (int k = 0; k < 2; k++) {
    ...
    __try {
        RoadAtFactory(rcx, r8, isUpgrade);
    } __except (EXCEPTION_EXECUTE_HANDLER) {

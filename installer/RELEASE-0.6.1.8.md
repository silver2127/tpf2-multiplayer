Companies survive a save made outside the lobby
...
Fixes
- A companies-mode world loaded in single player (or on a machine where the record could not be applied) and then saved lost every company: the save was written as a co-op world. The save now keeps the companies record it was loaded with, and the next machine that can apply it gets the companies back.
- The lobby's company assignment file no longer outlives the lobby, so a single-player load reads the save's own companies instead of the last lobby's roster.

...
- From 0.6.1.13: a resync no longer fails under Proton with "Could not pause all games ... No such file or directory: '\tmp\tpf2mp-data\...'". A `TPF2MP_DATADIR` launch option with a path that has no drive letter is now made absolute once, in the game, so the lobby and the game use the same folder. Removing that launch option also works; the default folder is fine under Proton.
...
If both fixes work for you, say so on the issue or in Discord and this becomes the next regular release.
